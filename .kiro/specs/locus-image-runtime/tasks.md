...
- [ ] 0.1 Create the `locus` package (CLI) depending on `locus_engine`
...
- [ ] 1.1 Implement `ArtifactType`, `ArtifactKind`, `Compat`, and `LocusArtifact`
...
- [ ] 1.2 Implement `ImageManifest`, `StageSpec`, `SourceSpec`, `Locusfile`, `PortSpec`
...
- [ ] 2.1 Implement `LocusfileLoader` (YAML load + model validation)
  - Single-image shorthand → one-stage pipeline; invalid setting → ConfigError.
  - _Requirements: 3.1, 3.6, 3.7_
- [ ] 2.2 Implement credential safety checks
...
- [ ] 4.1 Implement `PipelinePlanner`: graph build + cycle detection + waves
  - Topological ordering; parallelizable wave grouping; cycle → ConfigError.
  - _Requirements: 5.1, 5.2, 5.3, 5.4, 5.5, 5.6_
- [ ] 4.2 Implement static interchange type-check across edges
  - Major mismatch fails; minor warns; runs before any stage executes.
  - _Requirements: 6.3, 6.4, 6.5, 6.6_
- [ ] 4.3 Implement `StageExecutor` over the plan (process backend)
...
- [ ] 11.1 Correctness-property suite (Properties 1-8) in CI
  - _Requirements: 5.6, 6.4, 7.x, 4.x, 10.8, 12.x_
- [ ] 11.2 Golden two-stage pipeline integration test
  - _Requirements: 5.x, 6.x, 7.x_
- [ ] 11.3 Coverage, typing, docs; CLI usage examples
  - _Requirements: 1.1_
